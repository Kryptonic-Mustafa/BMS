...
files = {
    # 1. FIX THE DATABASE DRIVER: Revert to pool.query for TiDB compatibility
    "lib/db.ts": """
...
""",
    ...
    # 2. LOGIN API WITH LOGS: To verify database role fetching
...
""",
    ...
    # 3. FRONTEND LOGIN LOGS: To ensure the browser obeys the API
...
def apply_db_debug_fix():
    for path, content in files.items():
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write(content.strip())
    print("✅ DB Query Fix Applied & Debug Console Logs Injected!")
