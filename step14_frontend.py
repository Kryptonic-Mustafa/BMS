...
export default function MasterSettings() {
  ...
  const handleBulkDelete = async () => {
    const result = await Swal.fire({
        ...
        text: 'This cannot be undone. All their data will be lost.',
        icon: 'warning',
  ...
  const resetSystem = async () => {
    const res = await Swal.fire({
        ...
        text: 'This will delete ALL users, transactions, and data. Only YOU (Super Admin) will remain.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
      ...
      <Modal isOpen={isCustomerModalOpen} onClose={() => setModalOpen(false)} title="Customer Manager">
        <div className="flex flex-col h-[70vh]">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-2 mt-2">
                {filteredCustomers.map((c: any) => (
                    <div key={c.id} className={`border rounded-lg overflow-hidden ${selectedIds.includes(c.id) ? 'border-blue-500 ring-1 ring-blue-500' : 'border-slate-200 dark:border-slate-700'}`}>
                        <div className="p-3 bg-white dark:bg-slate-900 flex items-center gap-3">
                            <button onClick={(e) => { e.stopPropagation(); toggleSelectOne(c.id); }}>
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2">
                                     <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">{c.account_number}</span>
                ...
                {filteredCustomers.length === 0 && <p className="text-center text-slate-500 py-4">No matching customers found.</p>}
            </div>
        </div>
      </Modal>
    </div>
...
def frontend_upgrade():
    print("🎨 Installing Bulk Customer Manager...")
