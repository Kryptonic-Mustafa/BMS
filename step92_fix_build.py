...
files = {
    # 1. FIXED IMPORT: Changed back to @/lib/auth
...
""",
    ...
    # 2. FIXED ROUTE: Placed the UI in the correct (auth) folder
...
def fix_build():
    # 3. DELETE THE DUPLICATE FOLDER CAUSING THE CONFLICT
    if os.path.exists('app/login'):
        shutil.rmtree('app/login')
        print("✅ Deleted duplicate app/login route.")

    for path, content in files.items():
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write(content.strip())
    print("✅ Fixed Import Paths and Restored Login Page correctly.")
