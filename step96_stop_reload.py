...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    // 1. THIS IS THE MAGIC LINE: It stops the browser from refreshing the page
    e.preventDefault(); 
    
    console.log("🔥 [FRONTEND] Login button clicked! Default reload prevented.");
    ...
    try {
      console.log(`🔥 [FRONTEND] Sending fetch request to API for: ${email}`);
      ...
      console.log(`🔥 [FRONTEND] Push router to: ${data.redirectUrl}`);
      // 2. USE NEXT.JS ROUTER: Much faster and more stable than window.location.href
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-800 rounded-2xl shadow-2xl p-8 border border-slate-700">
        ...
        {/* THE FIX: A proper HTML form so the browser knows exactly what to do */}
